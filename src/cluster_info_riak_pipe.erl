...
cluster_info_generator_funs() ->
    [
     {"Riak Pipelines", fun pipelines/2},
     {"Riak Pipeline Vnode Queues", fun queues/2}
    ].

%% @doc Print information about the active pipelines.
-spec pipelines(ClusterInfoHandle::term(), fun()) -> ok.
pipelines(C, RC) ->
    Pipes = RC(riak_pipe_builder_sup, builder_pids, []),
    cluster_info:format(C, "Pipelines active: ~b~n", [length(Pipes)]),
    [ pipeline(C, Pipe, RC) || {_Id, Pipe, _Type, _Mods} <- Pipes],
    ok.

%% @doc Print information about the given pipeline.
%%      The pid given should be that of the builder of the pipeline.
-spec pipeline(ClusterInfoHandle::term(), Builder::pid(), fun()) -> ok.
pipeline(C, Pipe, RC) ->
    case RC(riak_pipe_builder, fitting_pids, [Pipe]) of
        {ok, Fits} ->
            cluster_info:format(C, " - ~p fittings: ~b alive~n",
                                [Pipe, length(Fits)]),
            [ fitting(C, Fit, RC) || Fit <- Fits ];
        gone ->
...
-spec fitting(ClusterInfoHandle::term(), Fitting::pid(), fun()) -> ok.
fitting(C, Fit, RC) ->
    case RC(riak_pipe_fitting, workers, [Fit]) of
        {ok, Workers} ->
...
-spec queues(ClusterInfoHandle::term(), fun()) -> ok.
queues(C, RC) ->
    VnodePids = RC(riak_core_vnode_master, all_nodes, [riak_pipe_vnode]),
    cluster_info:format(C, "Vnodes active: ~b~n", [length(VnodePids)]),
    [ queues(C, V, RC) || V <- VnodePids],
    ok.

%% @doc Print information about the workers on the given vnode.
-spec queues(ClusterInfoHandle::term(), Vnode::pid(), fun()) -> ok.
queues(C, V, RC) ->
    {Partition, Workers} = RC(riak_pipe_vnode, status, [V]),
