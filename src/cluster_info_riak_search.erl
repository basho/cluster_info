%% -------------------------------------------------------------------
%%
%% Riak Search: A decentralized text indexing store.
%%
...
-module(cluster_info_riak_search).
...
cluster_info_generator_funs() ->
    [
     {"Riak Search status", fun status/2}
    ].

status(C, _) -> % CPid is the data collector's pid.
    cluster_info:format(C, "status gathering incomplete\n", []).
